class App {
  ...
  send(message) {
    console.log('app sending', JSON.stringify(message));
    $.ajax({
  ...
  fetch() {
    $.ajax({
      ...
      success: (data) => {
        ...
        if (this.messages.results.length) {
  ...
  renderMessage() {
    ...
    var i = this.index;
    console.log('rendering messages ', this.messages.results);
    for (i; i >= 0; i--) {
      var ele = this.messages.results[i].data;
